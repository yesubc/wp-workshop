...
function infoway_optionsframework_add_admin() {
    ...
    $of_page = add_theme_page($themename, __('Theme Options', 'infoway'), 'edit_theme_options', 'optionsframework', 'infoway_optionsframework_options_page', 'div');
...
function infoway_optionsframework_options_page() {
    ...
    <div class="trail-notify">
        <?php $inktheme_url = 'http://www.inkthemes.com/wp-themes/infoway-wordpress-theme-with-built-in-lead-capture/'; ?>
        <h1><?php _e('Get InfoWay Theme PRO!', 'infoway'); ?></h1>
        <p style="font-size:15px; line-height: 20px;"><?php _e('You are Using the Lite Version of InfoWay Theme. Upgrade to Pro for extra features like Lead Capture Integration, Home Page Slider, Contact Page, Gallery Page, FullWidth Page Templates, Multiple Color Options and much more.', 'infoway'); ?></p>
        <a href="<?php echo esc_url($inktheme_url); ?>" target="blank"><?php _e('Upgrade to Infoway PRO Theme here >>', 'infoway'); ?></a>
    </div>	
    <div class="wrap" id="of_container">
        <div id="of-popup-save" class="of-save-popup">
            <div class="of-save-save"><?php _e('Options Updated', 'infoway'); ?></div>
        </div>
        <div id="of-popup-reset" class="of-save-popup">
            <div class="of-save-reset"><?php _e('Options Reset', 'infoway'); ?></div>
        </div>
        <form action="" enctype="multipart/form-data" id="ofform">
            <?php wp_nonce_field('theme-update-option'); ?>
            <div id="header">
                <div class="logo">
                    <h2><?php echo $themename; ?><?php _e(' Options', 'infoway'); ?></h2>
                </div>
                <a href="http://www.inkthemes.com" target="_blank">
            ...
            <div class="save_bar_top">
                ...
                <input type="submit" value="<?php _e('Save All Changes', 'infoway'); ?>" class="button-primary" onclick="tinyMCE.triggerSave()" />
        </form>
        <form action="<?php echo esc_attr($_SERVER['REQUEST_URI']) ?>" method="post" style="display:inline" id="ofform-reset">
            <span class="submit-footer-reset">
                <input name="reset" type="submit" value="<?php _e('Reset Options', 'infoway'); ?>" class="button submit-button reset-button" onclick="return confirm('Click OK to reset. Any settings will be lost!');" />
                <input type="hidden" name="of_save" value="<?php _e('reset', 'infoway'); ?>" />
            </span>
        </form>
    </div>
...
function infoway_optionsframework_uploader_function($id, $std, $mod) {
    ...
    if ($mod != 'min') {
        $val = $std;
        if (infoway_get_option($id) != "") {
            $val = infoway_get_option($id);
        }
        $uploader .= '<input class=\'of-input\' name=\'' . $id . '\' id=\'' . $id . '_upload\' type=\'text\' value=\'' . str_replace("'", "", $val) . '\' />';
    }
    $uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="' . $id . '">' . __('Upload Image', 'infoway') . '</span>';
    if (!empty($upload)) {
        $hide = '';
    } else {
        $hide = 'hide';
    }
    $uploader .= '<span class="button image_reset_button ' . $hide . '" id="reset_' . $id . '" title="' . $id . '">' . __('Remove', 'infoway') . '</span>';
    $uploader .='</div>' . "\n";
